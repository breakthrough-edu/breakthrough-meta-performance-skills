...
⚠️ **The same ceiling sits on the workflow's other request, and this check does not look at it.** The sync also calls the ads endpoint for each ad's status, and that call takes a row limit the same way and follows no cursor either. ⚠️ **Graded: the ceiling is measured off that step's own URL, which carries the limit in plain sight; what Meta does to an account that exceeds it was read out of the documentation and never observed on this endpoint.** ⇒ **An account big enough to paginate Insights is big enough to truncate that one too**, and the symptom is different enough to misread: the daily rows arrive complete, while some ads keep a stale status and a stale synced-at time. ⭐ **Apply the same three options to it**, and note that when the third option is the honest answer for Insights it is the honest answer for both.
...
⭐ **Grade this section before you lean on it, because its Lark half and its Meta half are not the same grade of fact.** **Measured 2026-08-24**, on a throwaway copy whose step list was byte-identical to the template's apart from its trigger and its two request URLs: the day gate holding in both directions, the loops, the link-state upsert, and every column the sync fills arriving filled. ⚠️ **Unverified: everything on Meta's side of those two requests.** **Both HTTP steps were pointed at a stand-in endpoint for every measurement that exists**, and as of 2026-08-24 the template's own workflow had never been run against Meta at all. What was checked on the template is that the steps are in it and read back as written, which is a structural check and not a run. ⛔ **So on a failure at this gate, the two requests are suspects on the same footing as the student's token and the student's account**, and one of the two is weaker than the other (step 1). What would settle it: one install's first manual run, with each HTTP step's actual response body recorded.
...
1. **Find every placeholder by reading the workflow, and let the workflow tell you how many there are.** ⛔ **Do not carry a count into this step**, and do not assume their names either. **Read how many HTTP steps this workflow has**, then read each step for its own: each one carries **a request URL** (it holds the ad account) and **a token**. Replace all of them with the student's values.
   ...
   ⚠️ **Unverified, and this is the weaker of the two requests: the second one's shape came out of Meta's documentation, not out of a call.** Its URL, its query parameters, the status field name it asks for, and the reply shape it expects (a `data` array beside a `paging` object) were all composed from the docs, and **no live call has ever been made to that endpoint from this build.** What is measured is only the Lark side, and it was measured against a stand-in endpoint: a step of that shape validates, runs, and writes the columns its sample response declares. ⇒ ⛔ **So when the ad layer's status and synced-at columns do not move, read that step's URL and its actual response body first**, against Meta's current documentation, before you go looking at the student's select options or their token. A wrong field name and a wrong reply shape both arrive as the same silent nothing described above. What would settle it: one call to that endpoint with a real token, and the response recorded.
