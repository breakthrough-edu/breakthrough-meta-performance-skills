# breakthrough-meta-performance-skills

Two Claude Code skills for a student running Meta ads for their own business.

| Skill | Job |
|---|---|
| `breakthrough-performance-db-setup` | One time. Installs the Lark Base template on the student's own tenant, wires the Meta sync, and files a pointer note in their vault. |
| `breakthrough-performance-analysis` | Weekly. Reads that Base, refuses to advise when the data cannot support advice, and classifies what is actually wrong. |
...
```
npx skills add -g breakthrough-edu/breakthrough-meta-performance-skills
```
...
`breakthrough-performance-db-setup/references/changing-the-base.md` and `breakthrough-performance-analysis/references/changing-the-base.md` are **the same law shipped twice**, because each skill has to carry it into context alone.

**They are byte-identical.** `diff` the two files; anything it returns is a bug:

```
diff breakthrough-performance-db-setup/references/changing-the-base.md \
     breakthrough-performance-analysis/references/changing-the-base.md
```
...
The vault note the setup writes and the dated verdict snapshot the analysis appends to that same note are **one contract described in two files**: `breakthrough-performance-db-setup/references/vault-note.md` and `breakthrough-performance-analysis/references/verdict-snapshot.md`.
