...
description: >
  ...
  daily sync to their own Meta token, reconcile it against Ads Manager, and
  leave a pointer note in their second-brain vault so the weekly analysis can
...
**7. Wire the sync.** Two placeholders replaced, workflow enabled, run once by hand, and rows actually present in Ad Daily afterwards. Pass test is rows on the table, not a green run. ⛔ Never read "the workflow ran" as "the data arrived".
